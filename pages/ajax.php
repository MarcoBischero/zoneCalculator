	...
	$resultDelCal=mysql_query($queryDelCal, $conn);
		for ($col=0;$col<6;$col++){
			for($row=0;$row<6;$row++){
				...
				$intersection=$colString.$rowString;
				//$data['intersection'.$col.$row]=$intersection;
				//$data['intersection'.$col.$row] = $cookies[''.$colString.$rowString];
				$pastoID=$cookies[00][$intersection];
				//$data['passtoID']=$pastoID;
				//$data['passtoID']=$cookies[0][23];
				if($error != true){
					//while($error == false){
